...
describe('random recommendation duration regression', () => {
  ...
  it('keeps the longest 42-step recommendation sequence slightly slower but within 3 seconds', () => {
    const totalDuration = Array.from({ length: 42 }, (_, index) => getRandomTickDelay(index))
      .reduce((total, delay) => total + delay, 0);

    expect(totalDuration).toBe(2_771);
    expect(totalDuration).toBeGreaterThan(2_500);
    expect(totalDuration).toBeLessThanOrEqual(3_000);
  });
});

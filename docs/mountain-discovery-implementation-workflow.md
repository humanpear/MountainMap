...
| 11 | PR 준비 | 진행 중 | 10 | `$ship` 리뷰 수정·검증 완료, 수정 커밋 후 재실행 대기 |
...
- 범위: `$ship`을 세 차례 실행해 `origin/main` 병합, 전체 검증, 구현 계획 감사, 테스트 커버리지 감사와 적대적 코드 리뷰를 수행했다. 첫 리뷰의 등정 데이터 로딩·결과 집합·모바일 시트·난이도 집계 문제, 두 번째 리뷰의 필터 포커스·중첩 시트·Kakao overlay·난이도 갱신 문제에 이어 세 번째 리뷰에서 발견한 모바일 선택 패널→전체 산 상세→브라우저 Back의 URL·화면 불일치를 수정했다.
...
- 검증: `origin/main` 병합 성공, `npm run lint` 통과, `npm test` 23개 파일·175개 테스트 통과, `npm run build` 통과, `npm run audit:guides` 정책 위반 0건. 최신 계획 감사는 48개 항목 중 45개 완료·1개 변경·2개 부분 완료·미구현 0개였고, 테스트 커버리지 감사는 93%(28/30)였다. 세 번째 적대적 리뷰의 medium 1건을 수정하고 전체 상세에서 선택 패널로 돌아오는 회귀 테스트를 추가했다.
- 결정: 로그인 사용자의 완료·미등정 필터는 등정 기록 조회가 `ready`일 때만 활성화한다. 결과 구성원이 바뀌어 선택한 산이 제외되면 결과 목록으로 복귀하고 지도를 새 결과에 맞춘다. 모바일 필터·결과·상세 시트는 필터와 패널을 구분한 임시 history layer를 사용하며, 중첩 필터의 적용·취소 action은 `popstate` 도착 시 실행해 아래 결과·상세 시트를 유지한다. 선택 패널 위 전체 산 상세에서 Back으로 패널 history layer에 복귀할 때는 전체 상세 route만 닫고 선택 패널을 유지한다. Kakao overlay는 표시 산 집합이 바뀔 때만 생성하고 선택·랜덤 강조·등정 표시는 기존 DOM과 z-index만 갱신한다. 난이도 백그라운드 갱신 실패 시 dirty 상태를 복구해 다음 지도 복귀에서 다시 조회한다. 난이도 집계 응답은 100대 명산에 없는 ID와 중복 ID를 거부하며 DB도 `0000000001`~`0000000100`만 저장하도록 제약한다.
...
- [ ] 지역 필터가 복수 지역 산을 누락하지 않는다.
- [ ] 체감 난이도 평균과 평가 인원이 표시된다.
- [ ] 평가 없음과 조회 오류가 구분된다.
- [ ] 등정 필터가 로그인 상태를 정직하게 처리한다.
- [ ] 정렬은 결과 목록에서만 순서를 바꾼다.
- [ ] 지도와 결과 목록이 같은 산 집합을 표시한다.
- [ ] 결과 목록과 상세정보 왕복 시 상태와 스크롤이 유지된다.
- [ ] 랜덤 추천이 현재 필터 결과를 벗어나지 않는다.
- [ ] 결과 0개와 데이터 오류 상태에 복구 행동이 있다.
- [ ] 데스크톱과 모바일 핵심 흐름을 QA했다.
- [ ] `DESIGN.md`의 글자 크기, 터치 영역, 반응형 규칙을 지켰다.
- [ ] lint, test, build가 통과했다.
- [ ] 병합 전 `$review`의 high/medium 문제를 해결했다.
